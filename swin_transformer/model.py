...
def window_partition(x, window_size: int):
    ...
    # contiguous()，会强制拷贝一份tensor，让它的布局和从头创建的一模一样，但是两个tensor完全没有联系,不然x.permute与x用的是同一个向量
    windows = (
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask):
        ...
        if self.shift_size > 0:
            # roll沿着dim滚动,在H纬度向上滚动shift_size，在W纬度向左滚动shift_size
    ...
    def create_mask(self, x, H, W):
        ...
        for h in h_slices:
            for w in w_slices:
                img_mask[:, h, w, :] = cnt
                cnt += 1
        # [nW, Mh, Mw, 1]=[batch*num_window,window_h,window_w,C]
